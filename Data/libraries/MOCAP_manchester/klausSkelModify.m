function klausSkelModify(handle, channels, skel, classification, real_class)
...
title({['classified: ' classification], ['original class: ' real_class]});
...
% if (length(folder) > 0) 
%     dir=['./' folder '/frame', sprintf('%0.4d',frame),'.png'];
%     
%     %f = figure(1);
%     %print(f, '-r100','-dpng',dir)
%     
%     %imwrite(frame2im(getframe),dir,'png');
%     
%     f = figure(1);
%     imwrite(hardcopy(f, '-Dzbuffer', '-r0'),dir,'png');
% end
